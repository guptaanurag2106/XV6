...
int send_message(int s_pid, int r_pid, const char *msg){
  ...
  // Increment the message count for the receiver process
  msg_count[r_pid] += 1;
  ...
  // if (rec_proc->msg_count >= MAX_MSG_PER_PROC){
  //   cprintf("[ERROR] Too many messages\n");
  //   // panic("Too many messages\n");
  //   return -1;
  // }
  // for (int i=0; i<MSG_SIZE; i++){
  //   rec_proc->shared_memory[rec_proc->msg_count][i] = *(msg + i);
  // }
  // rec_proc->msg_count += 1;

  // Wake up the recipient process if it is sleeping
