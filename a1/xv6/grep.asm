...
printf(int fd, const char *fmt, ...)
{
 ...
 761:	0f 84 b8 00 00 00    	je     81f <printf+0xcf>
  ap = (uint*)(void*)&fmt + 1;
 767:	8d 45 10             	lea    0x10(%ebp),%eax
 76a:	83 c6 01             	add    $0x1,%esi
  write(fd, &c, 1);
 76d:	8d 7d e7             	lea    -0x19(%ebp),%edi
  state = 0;
 770:	31 d2                	xor    %edx,%edx
  ap = (uint*)(void*)&fmt + 1;
 772:	89 45 d0             	mov    %eax,-0x30(%ebp)
 775:	eb 37                	jmp    7ae <printf+0x5e>
 ...
 799:	e8 45 fe ff ff       	call   5e3 <write>
 ...
 7f9:	e8 e5 fd ff ff       	call   5e3 <write>
        putc(fd, c);
      } else {
 ...
 80d:	e8 d1 fd ff ff       	call   5e3 <write>
  for(i = 0; fmt[i]; i++){
 ...
 881:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 884:	e8 5a fd ff ff       	call   5e3 <write>
        ap++;
 ...
 8fb:	57                   	push   %edi
 8fc:	53                   	push   %ebx
 8fd:	e8 e1 fc ff ff       	call   5e3 <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 9cd:	0f 84 ad 00 00 00    	je     a80 <malloc+0xd0>
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 9d3:	8b 07                	mov    (%edi),%eax
    if(p->s.size >= nunits){
 9d5:	8b 48 04             	mov    0x4(%eax),%ecx
 9d8:	39 f1                	cmp    %esi,%ecx
 9da:	73 71                	jae    a4d <malloc+0x9d>
 9dc:	81 fe 00 10 00 00    	cmp    $0x1000,%esi
 9e2:	bb 00 10 00 00       	mov    $0x1000,%ebx
 9e7:	0f 43 de             	cmovae %esi,%ebx
  p = sbrk(nu * sizeof(Header));
 9ea:	8d 0c dd 00 00 00 00 	lea    0x0(,%ebx,8),%ecx
 9f1:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
 9f4:	eb 1b                	jmp    a11 <malloc+0x61>
 ...
 a07:	73 4f                	jae    a58 <malloc+0xa8>
    ...
    if(p == freep)
 a09:	8b 3d a0 12 00 00    	mov    0x12a0,%edi
 ...
 a1b:	e8 2b fc ff ff       	call   64b <sbrk>
  if(p == (char*)-1)
 ...
 a32:	e8 e9 fe ff ff       	call   920 <free>
  return freep;
 a37:	a1 a0 12 00 00       	mov    0x12a0,%eax
 ...
 a64:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 a67:	a3 a0 12 00 00       	mov    %eax,0x12a0
}
 ...
 a7e:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 a80:	c7 05 a0 12 00 00 a4 	movl   $0x12a4,0x12a0
 a87:	12 00 00 
    base.s.size = 0;
 a8a:	bf a4 12 00 00       	mov    $0x12a4,%edi
    base.s.ptr = freep = prevp = &base;
 a8f:	c7 05 a4 12 00 00 a4 	movl   $0x12a4,0x12a4
 a96:	12 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 a99:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 a9b:	c7 05 a8 12 00 00 00 	movl   $0x0,0x12a8
 aa2:	00 00 00 
    if(p->s.size >= nunits){
 aa5:	e9 32 ff ff ff       	jmp    9dc <malloc+0x2c>
